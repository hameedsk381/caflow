  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button id="add-invoice-btn" className="btn btn-primary" onClick={openCreate}><Plus size={15} /> New Invoice</button>
      </div>
      ...
      <div className="table-wrapper">
        {loading ? (
          ...
          <div className="empty-state">
            <div className="empty-state-icon"><Receipt size={24} style={{ color: 'var(--text-muted)' }} /></div>
            <h3>No invoices yet</h3><p>Create your first invoice</p>
          </div>
        ) : (
          <table>
            <thead><tr><th>Invoice #</th><th>Client</th><th>Amount</th><th>Tax</th><th>Total</th><th>Status</th><th>Due Date</th><th></th></tr></thead>
            <tbody>
              {invoices.map(inv => (
                <tr key={inv.id}>
                  ...
                  <td>{inv.due_date ? format(new Date(inv.due_date), 'dd MMM yyyy') : <span className="text-muted">—</span>}</td>
                  <td>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <span className="modal-title">{editInvoice ? 'Edit Invoice' : 'New Invoice'}</span>
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                ...
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  ...
                  <textarea className="form-input" rows={2} value={form.description || ''} onChange={e => setForm({ ...form, description: e.target.value })} style={{ resize: 'vertical' }} placeholder="Services rendered…" />
                </div>
                <div className="form-group">
                  <label className="form-label">Amount (₹) *</label>
                  <input className="form-input" type="number" required min="0" step="0.01" value={form.amount || ''} onChange={e => setForm({ ...form, amount: e.target.value })} placeholder="0.00" />
                </div>
                <div className="form-group">
                  <label className="form-label">Tax / GST (₹)</label>
                  <input className="form-input" type="number" min="0" step="0.01" value={form.tax_amount || '0'} onChange={e => setForm({ ...form, tax_amount: e.target.value })} />
                </div>
                {form.amount && (
                ...
                <div className="form-group">
                  ...
                  <select className="form-input" value={form.status || 'draft'} onChange={e => setForm({ ...form, status: e.target.value })}>
                    {STATUSES.map(s => <option key={s}>{s}</option>)}
                  </select>
                </div>
                <div className="form-group">
